...
import com.suitmedia.bundle.sample.base.ui.recyclerview.BaseRecyclerView
import com.suitmedia.bundle.sample.helper.CommonLoadingDialog

abstract class BaseActivityWithDynamicFeature : AppCompatActivity(), MvpView {
    ...
    val listener = SplitInstallStateUpdatedListener { state ->
        ...
        state.moduleNames().forEach { name ->
            when (state.status()) {
                SplitInstallSessionStatus.DOWNLOADING -> onDownloadModule(name,state)
                SplitInstallSessionStatus.REQUIRES_USER_CONFIRMATION -> onRequiresUserConfirmation(name,state)
                SplitInstallSessionStatus.INSTALLED -> onInstalled(name,state)
                SplitInstallSessionStatus.INSTALLING -> onInstalling(name,state)
                SplitInstallSessionStatus.FAILED -> onFailed(name,state)
            }
        }
    }
    ...
    abstract fun onDownloadModule(name:String,state:SplitInstallSessionState)
    abstract fun onRequiresUserConfirmation(name:String,state:SplitInstallSessionState)
    abstract fun onInstalled(name:String,state:SplitInstallSessionState)
    abstract fun onInstalling(name:String,state:SplitInstallSessionState)
    abstract fun onFailed(name:String,state:SplitInstallSessionState)
